  ...
  <div>
    ...
    <b-table class="gap-top" small striped hover :items="items" :fields="fields">
      ...
      <template slot="tindakan" slot-scope="data">
        <b-button-group>
          <b-button v-b-modal.modal1 @click="selectedItem=data.item" size="sm" variant="primary"><icon name="pencil"></icon> Edit</b-button>
          <b-button @click="deleteItem(data.item)" size="sm" variant="danger"><icon name="remove"></icon> Delete</b-button>
        </b-button-group>
      </template>
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$http.delete('/transaksi/' + item.id).then(response => {
            this.$swal('Success', 'Your item has been deleted', 'success')
            this.reload()
